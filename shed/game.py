...
class ShedGame:
    ...
    def step(self, action: str):
        ...
        if action != "stand":
            ...
            (
                self.players[self.game_pointer].status,
                self.players[self.game_pointer].score,
            ) = self.judger.judge_round(self.players[self.game_pointer])
            if self.players[self.game_pointer].status == "bust":
                ...
                if self.game_pointer >= self.num_players - 1:
                    while self.judger.judge_score(self.dealer.hand) < 17:
                        self.dealer.deal_card(self.dealer)
                    self.dealer.status, self.dealer.score = self.judger.judge_round(
                        self.dealer
                    )
                    for i in range(self.num_players):
            ...
            action == "stand"
        ):  # If stand, first try to pass the pointer, if it's the last player, dealer deal for himself, then judge game for everyone using a loop
            (
                self.players[self.game_pointer].status,
                self.players[self.game_pointer].score,
            ) = self.judger.judge_round(self.players[self.game_pointer])
        ...
        for i in range(self.num_players):
            next_state["player" + str(i) + " hand"] = [
                card.get_index() for card in self.players[i].hand
            ]
